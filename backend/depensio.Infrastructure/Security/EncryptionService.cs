...
public class EncryptionService(IKeyManagementService _keyManagementService) : IEncryptionService
{
    ...
    public string Encrypt(string plainText)
    {
        ...
        { 
            ...
            { 
              ms.Write(aes.IV, 0, aes.IV.Length); // Stoke l'IV au debut du flux    
              ms.Write(BitConverter.GetBytes(keyVersion.Length), 0, keyVersion.Length); // Stoke l'IV au debut du flux
              ms.Write(Encoding.UTF8.GetBytes(keyVersion), 0, keyVersion.Length); // Stoke l'IV au debut du flux
